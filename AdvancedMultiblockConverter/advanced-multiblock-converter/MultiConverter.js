...
//#region Electron app Integration
/**
 * This function simulates file system operations in memory, sets up the required
 * command-line argument, and invokes the main conversion logic. It restores the
 * original process arguments after execution. If the conversion is successful,
 * it returns the contents of "output.js" from the in-memory file system.
 * Otherwise, it returns an error message.
 *
 * @param {string} inputJson - The JSON string to be used as input for the conversion.
 * @param {string} command - The command-line argument to control the conversion process.
 * @returns {string} - The contents of "output.js" if successful, or an error message if not.
 */
function runConversion(inputJson, command) {
  memoryFS["input.json"] = inputJson;
  ...
  try {
    ...
    return `Error running conversion: ${err.message}`;
  }

  process.argv[2] = oldArgv;

  if ("output.js" in memoryFS) {
    return memoryFS["output.js"];
  } else {
